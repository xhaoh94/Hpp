...
class MatchesState extends LineBasedState {
    ...
    update(update) {
        // 仅在选区 / 文档 / 折叠变化时重算，避免每次渲染都全量扫描
        const changed = update.selectionSet || update.docChanged || foldsChanged(update.transactions);
        ...
        if (sel.from !== sel.to) {
            ...
            // 跳过空白 / 过长选区；超大文档也跳过，避免卡顿
            if (text.length > 0 && text.length <= 200 && text.trim().length > 0) {
                const doc = update.state.doc.toString();
                if (doc.length <= 300000) {
                    let idx = doc.indexOf(text);
                    let count = 0;
                    while (idx !== -1 && count < 5000) {
                        this.set.add(update.state.doc.lineAt(idx).number);
                        idx = doc.indexOf(text, idx + text.length);
                        count++;
                    }
                }
            }
        ...
        this.map.clear();
        for (const ln of this.set) {
            this.map.set(ln, true);
        }
    }
    drawLine(ctx, lineNumber) {
        if (!this.get(lineNumber)) {
            return;
        }
        ...
        context.globalAlpha = 1;
        context.fillStyle = MATCH_COLOR;
        context.fillRect(0, offsetY, bar, lineHeight);
        context.fillRect(w - bar, offsetY, bar, lineHeight);
    }
}
function matches(view) {
    return new MatchesState(view);
}
...
class TextState extends LineBasedState {
    ...
    updateImpl(state, changes) {
        ...
        const highlightingCallback = () => {
            // An undo/redo or tab teardown may have replaced this document after
            // the callback was queued. Never publish stale line data.
            if (generation !== this._updateGeneration || this.view.state !== state) {
                return;
            }
            if (tree) {
                highlightTree(tree, highlighter, (from, to, tags) => {
...
const minimapClass = /*@__PURE__*/ViewPlugin.fromClass(class {
    ...
    update(update) {
        ...
        if (now) {
            ...
            this.matches.update(update);
            this.render();
        }
    }
    getWidth() {
        const cfgWidth = this.view.state.facet(Config).width;
        if (cfgWidth && cfgWidth > 0) {
            return cfgWidth;
        }
    ...
    render() {
        ...
        for (let i = startIndex; i < endIndex; i++) {
            ...
            this.matches.drawLine(drawContext, i + 1);
